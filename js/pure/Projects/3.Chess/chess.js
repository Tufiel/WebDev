...
let turn,eatablePieces=[],colors=[];

for(let i =1;i<65;i++)
colors.push(document.getElementById('s'+i).style.backgroundColor);
...
{
	if(document.getElementById('s'+i).textContent.split('').includes('K'))//if king
	  return;
...
function dangerForPieces()
{
	for(let i=0;i<eatablePieces.length;i++)
	 ...
	 setTimeout(
		()=>{
		for(let i=0;i<64;i++)
	    document.getElementById('s'+(i+1) ).style.backgroundColor = colors[i];
	 },1000);
}

function  checkForPromotion(p,id)
{
  if(p.textContent.toUpperCase().split('').includes('W') && id>56)
   p.textContent = 'WQ';
 if(p.textContent.toUpperCase().split('').includes('B') && id<9)
  p.textContent = 'BQ';
}
